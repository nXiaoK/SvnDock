...
- 工作区可选择 **隐藏仅换行符变化** 或 **隐藏仅空白变化**，默认显示全部差异。
  文件内容和 SVN 状态保持原样；属性、冲突及无法确认的项目仍会显示。隐藏文件在
  工作区发起的提交中默认不勾选，可在提交清单中重新勾选。字符串和缩进中的空白
  也可能有意义，请按需选择筛选范围。
...
- 删除整个目录时，提交预览直接显示删除范围，不自动加载全部子文件的删除内容。
  超大差异只预览明确标注的开头片段，可复制本次读取的完整输出；预览限制不会缩小
  实际提交到 SVN 的目录删除范围。
...
- 支持 Update、Commit、Add、Revert、Resolve、Cleanup，以及安全合并
  `svn:ignore` 规则。
- 提交与更新实时显示当前阶段、文件路径、SVN 已报告项数和用时；批量更新还会显示
  当前工作副本及已处理副本数。路径通知不代表上传完成百分比，文件传输、等待仓库
  确认和本地状态核验会分别提示。
...
在软件文件列表或访达的 SvnDock 右键菜单中，选择“还原至指定版本前…”。
干净文件未显示在变更列表时，可右键左侧工作副本，选择“还原文件至指定版本前…”
并选取文件；顶部“更多”菜单也提供该入口。
输入 `r100` 表示恢复到 `r99`，预览确认后同时还原内容与 SVN 属性，保留为
待提交的本地变更；可先检查差异再提交。文件需已纳管、在本地和目标版本中
存在，且没有未提交修改。详见[历史文件还原](Docs/Historical-File-Restore.md)。
