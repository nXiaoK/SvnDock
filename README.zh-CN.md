...
提交窗口底部提供“预览提交指令”。按当前勾选范围和提交说明展示执行目录、
最终 SVN 指令、标准输入及临时目标清单，支持复制完整预览。尚未填写说明时也能
查看指令，但实际提交仍要求有效说明。预览不会运行 SVN，返回后保留草稿和勾选。
详见[提交指令预览](Docs/Commit-Command-Preview.md)。

## 从 SVN 仓库检出

点击侧栏“从仓库检出…”（也可使用“更多”菜单或 `⌘N`），填写仓库 URL 和
完整本地路径。选择父目录后会建议一个项目目录名，可以自行修改。
目标可以是新目录或空目录，父目录需已存在；检出最新版本后自动加入工作副本列表。
检出过程中显示路径通知和已用时间，可以取消；失败时保留表单方便重试，不覆盖
已有文件。认证沿用本机 SVN 配置，不自动检出 `svn:externals`。
详见[检出说明](Docs/Checkout.md)。
...
- 支持检出（Checkout）和登记已有工作副本；尚未实现仓库导入（Import）。
